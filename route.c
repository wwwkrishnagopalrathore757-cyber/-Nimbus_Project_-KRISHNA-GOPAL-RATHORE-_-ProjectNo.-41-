...
static int choose_next_bin(const Bin *bins,
    ...
    for (int i = 0; i < bin_count; i++) {
        ...
        if (chosen_index == -1) {
            chosen_index = i;
            best_distance = dist;   // yaha sahi variable use karna tha
        } else {
            ...
            if (bins[i].priority > bins[chosen_index].priority) {
                ...
                if (dist < best_distance) {
                    chosen_index = i;
                    best_distance = dist;  // yaha pe pehle galat likha tha
                }
            }
        }
    }
...
RouteResult run_nearest_full_bin_route(Bin *bins,
    ...
    while (1) {
        ...
        // Route trace print kar rahe hain taaki output samajh aaye
    ...
    // Optionally: wapas depot ka distance bhi add kar sakte ho agar chaho
